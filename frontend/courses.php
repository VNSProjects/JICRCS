...
if(isset($_POST['submit'])){
    if(!empty($_POST['category'])) {
        ...
        $value=$_POST['course_id'];
        echo "$value";
        $link = 'python E:\xampp\htdocs\JICRCS\BackEnd\sources\add_course_rating.py ' . $value . " " . $email . " ". $selected;
...
$pdo = new PDO('mysql:host=localhost;dbname=JICRCS', 'root', '');
    ...
    $pdo = new PDO('mysql:host=localhost;dbname=JICRCS', 'root', '');
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = $pdo->prepare('select * from courseR');
    $sql->execute();
    $count2 = $sql->rowCount();
    #echo "$count2";
...
<div class="container-fluid">
    <div class="row align-items">
            <?php
                ...
                // }
                 if($count2 == 0){
                    $link = 'python E:\xampp\htdocs\JICRCS\BackEnd\sources\courses.py ' . $skill . " " . $email;
                    $output = shell_exec($link);
                    echo "$output";
                    $pdo = new PDO('mysql:host=localhost;dbname=JICRCS', 'root', '');
                    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    $stmt = $pdo->query("SELECT * FROM courseR");
                    while ($row = $stmt->fetch()) {
                        // component($row['title'], $row['uni'], "Online", $row['link']);
                        $id = $row['id'];
    ...
    <form class=\"h-100\" method=\"post\">
        <div class=\"card shadow h-100\" style=\"padding: 5%;\">
            ...
            <h2><strong class=\"mt-4\"> $course_title</strong></h2>
            <h5 style=\"color: grey;\">University:<strong>$level</strong></h5>
            <h5 style=\"color: grey;\">Type:<strong>$subject</strong></h5>
  ...
  <div class=\"accordion-item\">
    ...
    <button type=\"submit\" name=\"submit\" class=\"btn btn-success mt-4\">Give Ratings <i class=\"fa fa-shopping-cart\"></i></button>
    ...
    <form class=\"h-100\" method=\"post\">
        <div class=\"card shadow h-100\" style=\"padding: 5%;\">
            ...
            <h2><strong class=\"mt-4\"> $course_title</strong></h2>
            <h5 style=\"color: grey;\">Level:<strong>$level</strong></h5>
            <h5 style=\"color: grey;\">Subject:<strong>$subject</strong></h5>
            <button class=\"btn btn-success my-4\"><a style=\"text-decoration:none;color:#fff;\"href=\"$link\" target=\"_blank\">View Job</a> </button>
            <div class=\"accordion\" id=\"accordionExample\" >
  <div class=\"accordion-item\">
    <h2 class=\"accordion-header\" id=\"headingOne\">
