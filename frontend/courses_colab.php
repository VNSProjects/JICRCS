...
<div id="menu">
    ...
    <div class="container-fluid">
        <div class="row align-items">
            ...
            // }
            if ($count2 == 0) {
                $link = 'python E:\xampp\htdocs\JICRCS\BackEnd\sources\courses_colab.py '. $email;
                #echo "$link";
                $output = shell_exec($link);
                echo "$output";
                $pdo = new PDO('mysql:host=localhost;dbname=JICRCS', 'root', '');
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stmt = $pdo->query("SELECT * FROM coursesCollab");
                while ($row = $stmt->fetch()) {
                    component($row['title'], "Udemy", $row['subject'], $row['empType']);
                    // $id = $row['id'];
