...
$pdo = new PDO('mysql:host=localhost;dbname=JICRCS', 'root', '');
    ...
    $pdo = new PDO('mysql:host=localhost;dbname=JICRCS', 'root', '');
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = $pdo->prepare('select * from internsR');
    $sql->execute();
    $count2 = $sql->rowCount();
    #echo "$count2";
...
<div class="container-fluid">
    <div class="row align-items">
            <?php
                ...
                if($count == 0){
                 ...
                 if($count2 == 0){
                    $link = 'python E:\xampp\htdocs\JICRCS\BackEnd\sources\internships.py ' . $skill . " " . $email;
                    #echo "$link";
                    $output = shell_exec($link);
                    #echo "$output";
                    $pdo = new PDO('mysql:host=localhost;dbname=JICRCS', 'root', '');
                    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    $stmt = $pdo->query("SELECT * FROM internsR");
                    while ($row = $stmt->fetch()) {
                        //component($row['title'], $row['company'], $row['location'], $row['link']);
                        $id = $row['id'];
                        $ints_title = $row['title'];
                        $link = $row['link'];
                        $company = $row['company'];
    ...
    <form class=\"h-100\" method=\"post\">
        <div class=\"card shadow h-100\" style=\"padding: 5%;\">
            ...
            <h2><strong class=\"mt-4\"> $ints_title</strong></h2>
            <h5 style=\"color: grey;\">Company:<strong>$company</strong></h5>
