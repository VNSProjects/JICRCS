...
df=pd.read_csv(r"E:\\xampp\\htdocs\\JICRCS\\BackEnd\data\\internships_ratings.csv", index_col=['ints_id', 'title', 'company', 'location', 'link'])
#df.head()
...
for i in range(0,len(recommendations)):
    mycursor.execute("CREATE TABLE IF NOT EXISTS IntsCollab (id VARCHAR(255), title VARCHAR(255),\
        company VARCHAR(255), location VARCHAR(255))")
    
    sql = "INSERT INTO IntsCollab (id, title,company,location) VALUES (%s,%s,%s,%s)"
