...
df=pd.read_csv(r"E:\\xampp\\htdocs\\JICRCS\\BackEnd\data\\jobs_ratings.csv", index_col=['jobs_id', 'Job Title', 'Company Name', 'Location', 'skills', 'Employment Type', 'Description'])
#df.head()
...
for i in range(0,len(recommendations)):
    mycursor.execute("CREATE TABLE IF NOT EXISTS JobsCollab (id VARCHAR(255), jobstitle VARCHAR(255),\
        companyname VARCHAR(255),location VARCHAR(255))")
    
    sql = "INSERT INTO JobsCollab (id, jobstitle,companyname,location) VALUES (%s,%s,%s,%s)"
    val = (recommendations[i][0],recommendations[i][1],recommendations[i][2],recommendations[i][3])
    #print(val)
